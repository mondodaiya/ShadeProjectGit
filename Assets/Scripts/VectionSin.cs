...
public class VectionSin : MonoBehaviour
{
    ...
    public bool STOPGO;     //TrafficLightのstopGo変数の格納用変数
    public float time_elapsed;  //白点を動かすタイマー
    public float time_out;  //白点を動かすタイマーの設定時間
    ...
    private IEnumerator Fall(){ 
        ...
        while(true){
            ...
            //時間で消す
            bool stopGo = script.stopGo;
             if(stopGo == true){    //青信号のとき
               STOPGO = true;
              color.a = 0.0f;
              gameObject.GetComponent<Renderer>().material.color = color;            
             }else if(dist_player > alfa_circle){   //赤信号のときでプレーヤーから遠いとき
                STOPGO = false;
                color.a = 1.0f;
                gameObject.GetComponent<Renderer>().material.color = color;
             }else{     //赤信号のときでプレーヤーから近いとき
                 STOPGO = false;
                 color.a = 0.0f;
                 gameObject.GetComponent<Renderer>().material.color = color;
             }
            ...
            // v.y = 1;
            this.transform.localScale = new Vector3(0.4f + sin * dist_ratio * 0.05f, 0.002f, 0.4f + sin * dist_ratio * 0.05f);
