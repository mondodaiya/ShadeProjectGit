...
public class Vection : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // if(Mathf.Abs(accelerationY) > 20.0f && OVRInput.Get(OVRInput.RawButton.RIndexTrigger) && onOff == false){ // Aボタンを押したら
        //     onOff = true;   //ボタンのフラグをONにする
        //     StartCoroutine("Fall"); //コルーチンを呼び出す
        // }else if(Mathf.Abs(accelerationY) > 20.0f && onOff == false){
        //     onOff = true;
    ...
    IEnumerator Fall(){
        ...
        while(true){    //ループ処理
            ...
            if(counter <= 80){
                 ...
                 var headingReturn = Pos - targetPos;
                 headingReturn.y = 0;
                //  var headReturn = headingReturn / 100.0f * 12.375f;
